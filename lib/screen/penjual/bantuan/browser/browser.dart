...
import 'package:idaman/components/bantuan_card.dart';
...
class _BodyState extends State<Browser> {
                    ...
                    child: SmartRefresher(
                      ...
                      child: ListView.separated(
                          ...
                          itemBuilder: (BuildContext ctx, index) {
                              return BantuanCard(product: produks[index]);
                          },
  ...
  Future<bool> cari(String query, {bool isRefresh = false}) async {
    ...
    await apiService.listBantuan(query,currentPage,offset).then((value) {
      if(currentPage==1) {
        produks = value.data!;
      }else{
  ...
  Future<bool> find(String query, {bool isRefresh = false}) async {
    String requery = "bantuan=$query";
    if(isRefresh){
      currentPage = 1;
    }
    APIService apiService = new APIService();
    await apiService.listBantuan(requery,currentPage,offset).then((value) {
      if(currentPage==1) {
        produks = value.data!;
      }else{
        produks.addAll(value.data!);
